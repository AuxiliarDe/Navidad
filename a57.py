...
usuario = {
    "email": "coordinadoratrade.almafit@gmail.com",
    "password": "Market2022*"
}
...
def fetch_data(api_token, offset, limit, from_date, until_date):
    url = f"https://api.dropi.co/api/orders/myorders?exportAs=orderByRow&orderBy=id&orderDirection=desc&result_number={limit}&start={offset}&from={from_date}&until={until_date}"
    headers = {
...
def fetch_historywallet_data(api_token, offset, limit, from_date, until_date):
    url = f"https://api.dropi.co/api/historywallet?orderBy=id&orderDirection=desc&result_number={limit}&start={offset}&from={from_date}&until={until_date}"
    headers = {
        "Authorization": "Bearer " + api_token,
        "Content-Type": "application/json"
    }
...
while len(all_data) < max_records:
    data = fetch_data(api_token, offset, limit, from_date, until_date)
    if not data:
...
if not all_data:
    ...
    for attempt in range(max_retries):
        try:
            dbx.files_upload(csv_buffer.getvalue().encode(), f"/A57.csv", mode=dropbox.files.WriteMode.overwrite)
            print(f"Archivo subido a Dropbox como /A57.csv")
...
for attempt in range(max_retries):
    try:
        dbx.files_upload(csv_buffer_historywallet.getvalue().encode(), f"/A57H.csv", mode=dropbox.files.WriteMode.overwrite)
        print(f"Archivo subido a Dropbox como /A57H.csv")
