...
def obtener_token_dropi(email, password):
    url = "https://api.dropi.ec/api/login"
    headers = {
...
usuario = {
    "email": "ecuadorlandings@gmail.com",
    "password": "L@ndings.Ecuador-24"
}
...
def fetch_historywallet_data(api_token, offset, limit, from_date, until_date):
    url = f"https://api.dropi.ec/api/historywallet?orderBy=id&orderDirection=desc&result_number={limit}&start={offset}&textToSearch=&type=null&id=null&identification_code=null&user_id=268592&from={from_date}&until={until_date}&wallet_id=0"
    headers = {
        "Authorization": "Bearer " + api_token,
        "Content-Type": "application/json"
    }
...
if not all_data:
    ...
    for attempt in range(max_retries):
        try:
            dbx.files_upload(csv_buffer.getvalue().encode(), f"/A37.csv", mode=dropbox.files.WriteMode.overwrite)
            print(f"Archivo subido a Dropbox como /A37.csv")
...
for attempt in range(max_retries):
    try:
        dbx.files_upload(csv_buffer_historywallet.getvalue().encode(), f"/A37H.csv", mode=dropbox.files.WriteMode.overwrite)
        print(f"Archivo subido a Dropbox como /A37H.csv")
