...
celda_token = "A9"  # Cambia esta celda según el token que necesites usar
...
if not all_data:
    ...
    for attempt in range(max_retries):
        try:
            dbx.files_upload(csv_buffer.getvalue().encode(), f"/A73.csv", mode=dropbox.files.WriteMode.overwrite)
            print(f"Archivo subido a Dropbox como /A73.csv")
...
for attempt in range(max_retries):
    try:
        dbx.files_upload(csv_buffer_historywallet.getvalue().encode(), f"/A73H.csv", mode=dropbox.files.WriteMode.overwrite)
        print(f"Archivo subido a Dropbox como /A73H.csv")
