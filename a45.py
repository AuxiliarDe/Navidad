...
usuario = {
    "email": "usuarioprueba1@capitalcorpsas.com",
    "password": "ECOMmillonario2024"
}
...
if not all_data:
    ...
    df_flattened = explode_and_normalize(df_filtered, 'orderdetails', ['orderdetails.product.id', 'orderdetails.product.id_lista', 'orderdetails.product.name', 'orderdetails.product.name_in_order', 'orderdetails.quantity'])
    df_flattened = explode_and_normalize(df_flattened, 'tags', ['tags.tag_name'])

    # Convertir columnas específicas a números decimales
    decimal_columns = ["total_order", "shipping_amount", "amount_earned_dropshipper", "amount_earned_supplier", "dropshipper_amount_to_win", "quantity", "orderdetails.quantity"]
    ...
    for attempt in range(max_retries):
        try:
            dbx.files_upload(csv_buffer.getvalue().encode(), f"/A45.csv", mode=dropbox.files.WriteMode.overwrite)
            print(f"Archivo subido a Dropbox como /A45.csv")
...
for attempt in range(max_retries):
    try:
        dbx.files_upload(csv_buffer_historywallet.getvalue().encode(), f"/A45H.csv", mode=dropbox.files.WriteMode.overwrite)
        print(f"Archivo subido a Dropbox como /A45H.csv")
