...
celda_token = "A5"  # Cambia esta celda según el token que necesites usar
...
def fetch_data(api_token, offset, limit):
    url = f"https://api.dropi.mx/api/orders/myorders?exportAs=orderByRow&orderBy=id&orderDirection=desc&result_number={limit}&start={offset}&textToSearch=&status=null&supplier_id=false&user_id=36746&from=2024-01-01&until=2024-07-31&filter_product=undefined&haveIncidenceProcesamiento=false&tag_id=&warranty=false&seller=null&filter_date_by=null&invoiced=null"
    headers = {
...
def fetch_historywallet_data(api_token, offset, limit, from_date, until_date):
    url = f"https://api.dropi.mx/api/historywallet?orderBy=id&orderDirection=desc&result_number={limit}&start={offset}&textToSearch=&type=null&id=null&identification_code=null&user_id=36746&from={from_date}&until={until_date}&wallet_id=0"
    headers = {
        "Authorization": "Bearer " + api_token,
        "Content-Type": "application/json"
    }
...
if not all_data:
    ...
    df_flattened = explode_and_normalize(df_flattened, 'tags', ['tags.tag_name'])
    ...
    for attempt in range(max_retries):
        try:
            dbx.files_upload(csv_buffer.getvalue().encode(), f"/A58.csv", mode=dropbox.files.WriteMode.overwrite)
            print(f"Archivo subido a Dropbox como /A58.csv")
...
for attempt in range(max_retries):
    try:
        dbx.files_upload(csv_buffer_historywallet.getvalue().encode(), f"/A58H.csv", mode=dropbox.files.WriteMode.overwrite)
        print(f"Archivo subido a Dropbox como /A58H.csv")
