...
def fetch_historywallet_data(api_token, offset, limit, from_date, until_date):
    url = f"https://api.dropi.mx/api/historywallet?orderBy=id&orderDirection=desc&result_number={limit}&start={offset}&textToSearch=&type=null&id=null&identification_code=null&user_id=268592&from={from_date}&until={until_date}&wallet_id=0"
    headers = {
        "Authorization": "Bearer " + api_token,
        "Content-Type": "application/json"
    }
...
if not all_data:
    ...
    for attempt in range(max_retries):
        try:
            dbx.files_upload(csv_buffer.getvalue().encode(), f"/A74.csv", mode=dropbox.files.WriteMode.overwrite)
            print(f"Archivo subido a Dropbox como /A74.csv")
...
for attempt in range(max_retries):
    try:
        dbx.files_upload(csv_buffer_historywallet.getvalue().encode(), f"/A74H.csv", mode=dropbox.files.WriteMode.overwrite)
        print(f"Archivo subido a Dropbox como /A74H.csv")
